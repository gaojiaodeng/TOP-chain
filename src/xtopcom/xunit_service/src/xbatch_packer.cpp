...
#define ENABLE_NEW_XIP_DELAY_CLOCK (2)
...
xbatch_packer::xbatch_packer(base::xtable_index_t                             &tableid,
  ...
  : xcsaccount_t(_context, target_thread_id, account_id), m_tableid(tableid), m_last_view_id(0), m_para(para), m_table_addr(common::xtable_address_t::build_from(account_id)) {
    ...
    xvip2_t empty_xip;
    set_empty_xip2(empty_xip);
    xcsaccount_t::reset_xip_addr(empty_xip);
...
void xbatch_packer::set_xip(data::xblock_consensus_para_t & blockpara, const xvip2_t & leader) {
    ...
    if (zone_id == base::enum_chain_zone_consensus_index || zone_id == base::enum_chain_zone_evm_index) {
        if (xcons_utl::is_auditor(leader)) {
            ...
            xunit_dbg("[xunitservice] set auditor leader account:%s validator:%s auditor:%s", get_account().c_str(), xcons_utl::xip_to_hex(blockpara.get_validator()).c_str(), xcons_utl::xip_to_hex(blockpara.get_auditor()).c_str());
        } else {
            ...
            xunit_dbg("[xunitservice] set validator leader account:%s validator:%s auditor:%s", get_account().c_str(), xcons_utl::xip_to_hex(blockpara.get_validator()).c_str(), xcons_utl::xip_to_hex(blockpara.get_auditor()).c_str());
        }
    } else {
...
std::shared_ptr<xproposal_maker_face> xbatch_packer::get_proposal_maker() const {
    return m_proposal_maker;
}
...
bool xbatch_packer::start_proposal(uint32_t min_tx_num) {
    ...
    set_xip(*m_leader_cs_para, get_xip2_addr());  // set leader xip
    ...
    if (leader_xip_changed(proposal_para)) {
        xwarn("xbatch_packer::start_proposal fail-leader xip changed.%s", proposal_para.dump().c_str());
        return false;
    }
...
void xbatch_packer::check_latest_cert_block(base::xvblock_t* _cert_block, uint64_t viewid, uint64_t clock, std::error_code & ec) {
    if ( (clock < _cert_block->get_clock()) // proposal clock should >= cert clock
        || (viewid <= _cert_block->get_viewid())) { // proposal viewid should > cert viewid
        ...
        xunit_warn("xbatch_packer::check_latest_cert_block fail-view behind,account=%s,current=%ld,%ld,cert=%ld,%ld",
            get_account().c_str(), viewid, clock, _cert_block->get_viewid(), _cert_block->get_clock());
        return;
    }
}
...
bool xbatch_packer::check_for_view_fire(base::xvblock_t * cert_block, uint64_t viewid, uint64_t clock) {
    auto ret = check_state_sync(cert_block);
    if (!ret) {
        return false;
    }

    std::error_code ec;
    check_latest_cert_block(cert_block, viewid, clock, ec);
    if (ec) {
        return false;
    }
    return true;
...
bool xbatch_packer::check_is_leader(uint64_t viewid, uint64_t cur_clock, base::xvblock_t * _cert_block) {
    auto accessor = m_para->get_resources()->get_data_accessor();
    auto leader_election = m_para->get_resources()->get_election();
    std::error_code ec;
    auto local_xip = get_xip2_addr();
    auto new_xip = get_new_xip(cur_clock);
    if (!is_xip2_empty(new_xip)) {
        local_xip = new_xip;
    }
    ...
    xvip2_t leader_xip = leader_election->get_leader_xip(viewid, get_account(), _cert_block, local_xip, local_xip, election_epoch, rotate_mode);
    bool is_leader_node = xcons_utl::xip_equals(leader_xip, local_xip);
    if (is_leader_node && !is_xip2_empty(new_xip)) {
        enable_new_xip(true);
    }
    xunit_info("xbatch_packer::on_view_fire is_leader=%d account=%s,tps_key viewid=%ld,clock=%ld,cert_height=%ld,cert_viewid=%ld,this:%p node:%s xip:%s,leader:%s,rotate_mode:%d",
            is_leader_node, get_account().c_str(), viewid, cur_clock, _cert_block->get_height(),
            _cert_block->get_viewid(), this, m_para->get_resources()->get_account().c_str(),
            xcons_utl::xip_to_hex(local_xip).c_str(), xcons_utl::xip_to_hex(leader_xip).c_str(), rotate_mode);

    return is_leader_node;
}
...
bool xbatch_packer::on_view_fire(const base::xvevent_t & event, xcsobject_t * from_parent, const int32_t cur_thread_id, const uint64_t timenow_ms) {
    ...
    m_raw_timer->stop();
    reset_leader_info();
    xdbg_info("xbatch_packer::on_view_fire account=%s,clock=%ld,viewid=%ld", get_account().c_str(), view_ev->get_clock(), view_ev->get_viewid());
    // auto local_xip = get_xip2_addr();
    ...
    if (_cert_block == nullptr) {
        xunit_error("xbatch_packer::on_view_fire fail-get cert block ,account=%s,viewid=%ld,clock=%ld", get_account().c_str(), view_ev->get_viewid(), view_ev->get_clock());
        return false;
    }

    // check if this node is leader
    bool is_leader_node = check_is_leader(m_last_view_id, m_last_view_clock, _cert_block.get());

    auto ret = check_for_view_fire(_cert_block.get(), view_ev->get_viewid(), view_ev->get_clock());
    if (!ret) {
        xunit_warn("xbatch_packer::on_view_fire fail-check for view fire,is_leader=%d,account=%s,viewid=%ld,clock=%ld,cert=%s",
            is_leader_node, get_account().c_str(), view_ev->get_viewid(), view_ev->get_clock(), _cert_block->dump().c_str());
        if (is_leader_node) {
...
bool xbatch_packer::check_state_sync(base::xvblock_t * cert_block) {
    ...
    if (base::xvchain_t::instance().need_store_units(get_zone_index())) {
        // storage node should not invoke sync, it need produce the whole mpt state
        xwarn("xbatch_packer::check_state_sync storgage node no need sync.block=%s,execute_height=%ld,zone_index=%d",cert_block->dump().c_str(), latest_executed_height, get_zone_index());
        return false;
    }
...
bool  xbatch_packer::on_timer_fire(const int32_t thread_id, const int64_t timer_id, const int64_t current_time_ms, const int32_t start_timeout_ms, int32_t & in_out_cur_interval_ms) {
    ...
    if (m_leader_cs_para == nullptr) {
        auto _cert_block = m_para->get_resources()->get_vblockstore()->get_latest_cert_block(get_account(), metrics::blockstore_access_from_us_on_view_fire);
        if (_cert_block == nullptr) {
            xunit_error("xbatch_packer::on_timer_fire fail-get cert block ,account=%s,viewid=%ld,clock=%ld", get_account().c_str(), m_last_view_id, m_last_view_clock);
            return true;
        }
        auto ret = check_for_view_fire(_cert_block.get(), m_last_view_id, m_last_view_clock);
        if (!ret) {
            xunit_warn("xbatch_packer::on_timer_fire fail-check for view fire,account=%s,viewid=%ld,clock=%ld,cert=%s",
                get_account().c_str(), m_last_view_id, m_last_view_clock, _cert_block->dump().c_str());
            m_raw_timer->start(m_pack_strategy.get_timer_interval(), 0);
            return true;
        }
        std::error_code ec;
        m_leader_cs_para = m_proposal_maker->leader_set_consensus_para_basic(_cert_block.get(), m_last_view_id, m_last_view_clock, ec);    
        if (nullptr == m_leader_cs_para) {
            xunit_warn("xbatch_packer::on_timer_fire fail-leader_set_consensus_para_basic. account=%s,viewid=%ld,clock=%ld,cert_height=%ld. error=%s",
                get_account().c_str(), m_last_view_id, m_last_view_clock, _cert_block->get_height(), ec.message().c_str());
            m_raw_timer->start(m_pack_strategy.get_timer_interval(), 0);
            return true;
        }
    }
...
bool xbatch_packer::send_out(const xvip2_t & from_addr, const xvip2_t & to_addr, const base::xcspdu_t & packet, int32_t cur_thread_id, uint64_t timenow_ms) {
    xunit_info("xbatch_packer::send_out pdu=%s,tps_key,body_size:%d,from_xip=%s,to_xip=%s,node_xip=%s,this:%p",
...
bool xbatch_packer::verify_proposal_viewid(const xvip2_t & from_addr, const xvip2_t & local_addr, uint64_t viewid) {
    bool valid = false;
    // step 1: verify viewid =》 [ local_viewid <= proposal_viewid < (local_viewid + 8)]
    auto proposal_view_id = viewid;
    if (m_last_view_id == 0 || (proposal_view_id >= m_last_view_id &&  proposal_view_id < (m_last_view_id + 8))) {
        ...
        if (!ec) {
            ...
            if (xcons_utl::xip_equals(leader_xip, from_addr)) {
                enable_new_xip_backup(local_addr);
                valid = true;
            }
        } else {
            // TODO here may happen when many elect blocks sync
            xunit_warn("xbatch_packer::verify_proposal_viewid xip=%s version from error", xcons_utl::xip_to_hex(from_addr).c_str());
        }
    } else {
...
bool xbatch_packer::recv_in(const xvip2_t & from_addr, const xvip2_t & to_addr, const base::xcspdu_t & packet, int32_t cur_thread_id, uint64_t timenow_ms) {
    ...
    xunit_info("xbatch_packer::recv_in,tps_key consensus_tableblock pdu_recv_in=%s, clock=%llu, viewid=%llu, from_xip=%s,to_xip=%s,node_xip=%s.",
    ...
    if (type == xconsensus::enum_consensus_msg_type_proposal || type == xconsensus::enum_consensus_msg_type_proposal_v2 || type == xconsensus::enum_consensus_msg_type_preproposal) {
        bool valid = verify_proposal_viewid(from_addr, to_addr, packet.get_block_viewid());
        if (!valid) {
            xunit_warn("xbatch_packer::recv_in fail-invalid msg tps_key,viewid=%ld,pdu=%s,at_node:%s,this:%p",
                m_last_view_id, packet.dump().c_str(), xcons_utl::xip_to_hex(to_addr).c_str(), this);
            return false;
        }
    }

    return process_msg(from_addr, to_addr, packet, cur_thread_id, timenow_ms);
}

int xbatch_packer::verify_proposal(base::xvblock_t * proposal_block, base::xvqcert_t * bind_clock_cert, xcsobject_t * _from_child) {
    auto ret = veriry_proposal_by_preproposal_block(proposal_block);
    if (ret != blockmaker::xblockmaker_error_proposal_no_preproposal) {
        return ret;
    }
    ...
    proposal_para.set_xip(proposal_block->get_cert()->get_validator(), proposal_block->get_cert()->get_auditor());
    proposal_para.set_drand_height(proposal_block->get_cert()->get_drand_height());
    set_election_round(false, proposal_para);
    auto local_block = m_proposal_maker->make_proposal_backup(proposal_block, proposal_para);
    if (local_block == nullptr) {
        return blockmaker::xblockmaker_error_proposal_backup_make_block_fail;
    }
    ret = m_proposal_maker->verify_proposal(proposal_block, local_block.get());
    if (ret == xsuccess) {
...
xvip2_t xbatch_packer::get_new_xip(uint64_t cur_clock) {
    if (is_xip2_empty(m_new_xip2)) {
        return m_new_xip2;
    }

    if (cur_clock < m_start_time + ENABLE_NEW_XIP_DELAY_CLOCK) {
        xdbg("xbatch_packer::get_new_xip not now. cur:%llu,start:%llu", cur_clock, m_start_time);
        xvip2_t empty_xip;
        set_empty_xip2(empty_xip);
        return empty_xip;
    }
...
void xbatch_packer::enable_new_xip_backup(const xvip2_t & to_xip) {
    if (is_xip2_equal(m_new_xip2, to_xip)) {
        enable_new_xip(false);
    }
}

void xbatch_packer::enable_new_xip(bool leader) {
    if (!is_xip2_empty(get_xip2_addr())) {
        m_last_xip2 = get_xip2_addr();
    }
    xinfo("xbatch_packer::enable_new_xip leader:%d %s %s,last xip:%s node:%s viewid=%llu this:%p",
          leader,
          get_account().c_str(),
...
bool xbatch_packer::reset_xip_addr(const xvip2_t & new_addr, common::xlogic_time_t start_time) {
    auto zone_id = get_zone_id_from_xip2(new_addr);
    if (zone_id != base::enum_chain_zone_consensus_index && zone_id != base::enum_chain_zone_beacon_index && zone_id != base::enum_chain_zone_zec_index &&
        ...
        xerror("xbatch_packer::reset_xip_addr fail-wrong zone id. zoneid=%d", zone_id);
        return false;
    }

    auto cur_xip = get_xip2_addr();
    xdbg("cur_xip:%s", xcons_utl::xip_to_hex(cur_xip).c_str());
    if (is_xip2_empty(cur_xip)) {
        xinfo("xbatch_packer::reset_xip_addr enable now %s %s node:%s this:%p",
          get_account().c_str(),
    ...
    xinfo("xbatch_packer::reset_xip_addr %s %s,last xip:%s node:%s viewid=%llu this:%p",
        get_account().c_str(),
        xcons_utl::xip_to_hex(m_new_xip2).c_str(),
        xcons_utl::xip_to_hex(m_last_xip2).c_str(),
        m_para->get_resources()->get_account().c_str(),
...
bool xbatch_packer::send_preproposal(const data::xblock_consensus_para_t & cs_para,
                                     const std::vector<data::xcons_transaction_ptr_t> & txs,
                                     const std::vector<base::xvproperty_prove_ptr_t> & receiptid_state_proves) {
    // do nothing for normal tables.
    return true;
}

xunit_service::xpreproposal_send_cb xbatch_packer::get_preproposal_send_cb() {
    return std::bind(&xbatch_packer::send_preproposal,
                     this,
                     std::placeholders::_1,
                     std::placeholders::_2,
                     std::placeholders::_3);
...
bool xbatch_packer::process_msg(const xvip2_t & from_addr, const xvip2_t & to_addr, const base::xcspdu_t & packet, int32_t cur_thread_id, uint64_t timenow_ms) {
    if (packet.get_msg_type() == xconsensus::enum_consensus_msg_type_preproposal) {
        // normal tables will not receive preproposal
        xerror("xbatch_packer::process_preproposal_msg received preproposal:%s", packet.dump().c_str());
        return true;
    }
    return xcsaccount_t::recv_in(from_addr, to_addr, packet, cur_thread_id, timenow_ms);
}

int xbatch_packer::veriry_proposal_by_preproposal_block(base::xvblock_t * proposal_block) {
    return blockmaker::xblockmaker_error_proposal_no_preproposal;
}

bool xbatch_packer::leader_xip_changed(const data::xblock_consensus_para_t & cs_para) const {
    xvip2_t local_xip = get_xip2_addr();
    if (cs_para.get_auditor().high_addr != local_xip.high_addr && cs_para.get_auditor().low_addr != local_xip.low_addr && cs_para.get_validator().high_addr != local_xip.high_addr &&
        cs_para.get_validator().low_addr != local_xip.low_addr) {
        return true;
    }
    return false;
}
