...
class xrpc_eth_parser_t {
 public:
    static  void log_to_json(xlog_location_t const& loglocation, evm_common::xevm_log_t const& log, Json::Value & js_v);
    ...
    static  void receipt_to_json_for_top_rpc(uint32_t tx_idx,  data::xeth_transaction_t const& ethtx,
                                             data::xeth_store_receipt_t const &evm_tx_receipt,Json::Value & js_v, std::error_code & ec);
    ...
    static  data::xtransaction_ptr_t json_to_ethtx(Json::Value const& request, data::eth_error& ec);
