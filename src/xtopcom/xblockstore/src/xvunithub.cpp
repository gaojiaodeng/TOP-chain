...
namespace top
{
    ...
    //                xassert(target_index->get_block_flags() == target_index->get_this_block()->get_block_flags());  // TODO(jimmy)
        ...
        base::xauto_ptr<base::xvblock_t>    xvblockstore_impl::get_block_by_hash(const std::string& hash)
        {
            ...
            xerror("xvblockstore_impl::get_block_by_hash, fail-load block.account=%s,height=%ld,hash=%s,bindexs=%zu",account.c_str(),height,top::to_hex_prefixed(top::to_bytes(hash)).c_str(),bindexs.get_vector().size());
            return nullptr;
        }
