...
std::shared_ptr<base::xinput_actions_cache_base> xtx_actions_cache_t::get_cache(base::xvblock_t * block) {
    ...
    // }
// #ifdef ENABLE_METRICS
//     XMETRICS_GAUGE(metrics::txpool_tx_action_cache, -erase_num);
//     xdbg("xtx_actions_cache_t::get_cache cache size:%d, reduce:%d", XMETRICS_GAUGE_GET_VALUE(metrics::txpool_tx_action_cache), erase_num);
// #endif
    return actions;
}
